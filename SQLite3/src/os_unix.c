...
#ifdef ENABLE_LOCKING_CALLBACKS
/*
** Code to support externalized locking API.
*/

// these are defined in pager.c
extern const char *_sqlite3_pager_file_path(struct Pager *pPager); //! caller must free() return value

static SQLite3OsGetLockingCallbacksCallback _sqlite3OsGetLockingCallbacksCallback = NULL;

void sqlite3OsSetLockingCallback(SQLite3OsGetLockingCallbacksCallback lockingCallbackFuncPtr){
    _sqlite3OsGetLockingCallbacksCallback = lockingCallbackFuncPtr;
}

SQLite3OsGetLockingCallbacksCallback sqlite3OsGetLockingCallback() {
  return _sqlite3OsGetLockingCallbacksCallback;
}

static void _setLockingCallbacks(OsFile *id) {
    const char *path = _sqlite3_pager_file_path(id->pPager);
    if (!path) {
        id->lockingCallbacks = NULL;
    }
    id->lockingCallbacks = _sqlite3OsGetLockingCallbacksCallback(path, id->h);
    sqliteFree(path);
    assert( id->lockingCallbacks );
}
#endif
...
#ifdef ENABLE_LOCKING_CALLBACKS
    {
        SQLite3OsLockingCallbacks *lockingCallbacks = id->lockingCallbacks;
        
        if (!lockingCallbacks && _sqlite3OsGetLockingCallbacksCallback) {
            _setLockingCallbacks(id); 
            lockingCallbacks = id->lockingCallbacks;
        }
        
        if (lockingCallbacks && lockingCallbacks->checkReservedFuncPtr) {
	  TRACE2("EXT TEST RESERVED %d\n", id->h);
            sqlite3OsEnterMutex();
            r = lockingCallbacks->checkReservedFuncPtr(lockingCallbacks->cachedPath, id->h, lockingCallbacks);
            sqlite3OsLeaveMutex();
            return r;
        }
    }  
...
int sqlite3OsLock(OsFile *id, int locktype){
  ...
  */
...
#ifdef ENABLE_LOCKING_CALLBACKS
  {
      ...
      if (lockingCallbacks && lockingCallbacks->lockFuncPtr) {
	TRACE7("EXT LOCK %d %s was %s(%s,%d) pid=%d\n", id->h, locktypeName(locktype), 
	       locktypeName(id->locktype), locktypeName(pLock->locktype), pLock->cnt
	       ,getpid() );
	
          sqlite3OsEnterMutex();
          int status = lockingCallbacks->lockFuncPtr(lockingCallbacks->cachedPath, id->h, locktype, lockingCallbacks);
          sqlite3OsLeaveMutex();
...
int sqlite3OsUnlock(OsFile *id, int locktype){
...
#ifdef ENABLE_LOCKING_CALLBACKS
  {
      ...
      if (lockingCallbacks && lockingCallbacks->unlockFuncPtr) {
	TRACE7("EXT UNLOCK %d %d was %d(%d,%d) pid=%d\n", id->h, locktype, id->locktype, 
	       id->pLock->locktype, id->pLock->cnt, getpid());
          sqlite3OsEnterMutex();
          int status = lockingCallbacks->unlockFuncPtr(lockingCallbacks->cachedPath, id->h, locktype, lockingCallbacks);
          sqlite3OsLeaveMutex();
          if (status == SQLITE_OK) {
              id->locktype = locktype;
          }          
...
#ifdef ENABLE_LOCKING_CALLBACKS
  if (id->lockingCallbacks && id->lockingCallbacks->free)
      id->lockingCallbacks->free(id->lockingCallbacks);
#endif
