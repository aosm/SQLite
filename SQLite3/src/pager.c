...
#ifdef ENABLE_LOCKING_CALLBACKS
const char *_sqlite3_pager_file_path(Pager *pPager) //! caller must free() return value
{
    if (!pPager) return NULL;
    if (pPager->zFilename && pPager->zDirectory) {
        char path[MAXPATHLEN];
        char *target;
        target = stpcpy(path, pPager->zDirectory);
        *target = '/';
        target++;
        *target = '\0';
        target++;
        target = sqliteMalloc( (target - path) + strlen(pPager->zFilename) + 1);
        strcpy(target, pPager->zFilename);
        return target;
    }
