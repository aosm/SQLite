...
static int unixSetFileProtection(int fd, int protFlags){
  int protClass = 4; /* default==SQLITE_OPEN_FILEPROTECTION_NONE */
...
(!defined(TARGET_IPHONE_SIMULATOR)||(TARGET_IPHONE_SIMULATOR==0))
  ...
  if( protFlags!=SQLITE_OPEN_FILEPROTECTION_NONE ){
    ...
    case SQLITE_REPLACE_DATABASE: {
      ...
      */
      ...
      if( !(srcWalFD<0) ){
        ...
        if( !(dstWalFD<0) ){
#if SQLITE_ENABLE_DATA_PROTECTION
          rc = unixSetFileProtection(dstWalFD, pFile->protFlags);
          if( rc ){
            if( rc==SQLITE_IOERR ){
              pFile->lastErrno = errno;
            }
...
#if SQLITE_ENABLE_DATA_PROTECTION
    rc = unixSetFileProtection(pShmNode->h, pDbFd->protFlags);
    if( rc ){
      if( rc==SQLITE_IOERR ){
        pDbFd->lastErrno = errno;
      }
      close(pShmNode->h);
      goto shm_open_err;
    }
