...
#if SQLITE_ENABLE_DATA_PROTECTION
static int unixSetFileProtection(int fd, int protFlags){
  int protClass;
  int currClass;
  ...
  switch (protFlags) {
    ...
    case SQLITE_OPEN_FILEPROTECTION_COMPLETEUNTILFIRSTUSERAUTHENTICATION:
      ...
      break;
    default:
      assert(protFlags == SQLITE_OPEN_FILEPROTECTION_NONE);
      protClass = 4;
  }
...
#if SQLITE_ENABLE_DATA_PROTECTION
  if( rc==SQLITE_OK ){
    rc = unixSetFileProtection(fd, protFlags);
  }
#endif
  if( rc==SQLITE_OK ){
    *pFd = fd;
    ...
    */
    ...
    || defined(__APPLE__))
      ...
      if( (pDbFd->ctrlFlags&UNIXFILE_RDONLY) == 0 ){
        ...
        if( shmProtFlags==SQLITE_OPEN_FILEPROTECTION_COMPLETE ){
          /* avoid munging shared memory once the shm is open */
          shmProtFlags = SQLITE_OPEN_FILEPROTECTION_COMPLETEUNLESSOPEN;
        }
        rc = unixSetFileProtection(pShmNode->h, shmProtFlags);
        if( rc ){
          if( rc==SQLITE_IOERR ){
...
#if SQLITE_ENABLE_DATA_PROTECTION
  if( !isReadonly ){
    rc = unixSetFileProtection(fd, p->protFlags);
    if( rc ){
      if( rc!=SQLITE_AUTH ){
        rc =SQLITE_CANTOPEN_BKPT;
      }
